...
export const auth = betterAuth({
  ...
  // `sendResetPassword` hooks into Better Auth's built-in reset flow:
  //   POST /api/auth/forget-password  → generates a token and calls this
  //   POST /api/auth/reset-password   → verifies the token and updates the hash
  // The token is stored in the `verification` table (0001_auth.sql) and expires
  // in 1 hour. `url` already contains the full reset link including the token.
